...
/**
 */
public class ActividadBuscador extends Buscador<Actividad> {

    private static final Logger log = LoggerFactory.getLogger(ActividadBuscador.class);

    private OrigenGeneral oGeneral;

    public ActividadBuscador(Frame parent, boolean modal) {
        super(parent, modal);
    }

    public ActividadBuscador(Dialog parent, boolean modal) {
        super(parent, modal);
    }

    public static ActividadBuscador mostrar(Component padre, Aplicacion app) {
        ActividadBuscador ab;
        Window mw = SwingUtilities.getWindowAncestor(padre);
        if (mw instanceof Frame) {
            ab = new ActividadBuscador((Frame) mw, true);
        } else {
            ab = new ActividadBuscador((Dialog) mw, true);
        }
        ab.initComponents();
        ab.init(app);
    ...
    private void initComponents() {
        ...
        jspItems.setViewportView(jlActividades);
    ...
    private void jbBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbBuscarActionPerformed
        try {
            modelo.setList(oGeneral.listarActividades(jtfNombre.getText()));
    ...
    private void jbAceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbAceptarActionPerformed
        try {
            aceptar(jlActividades.getSelectedIndex());
        } catch (AppException ex) {
