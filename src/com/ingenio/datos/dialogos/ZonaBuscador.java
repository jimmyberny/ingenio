...
/**
 */
public class ZonaBuscador extends Buscador<Zona> {

    private static final Logger log = LoggerFactory.getLogger(ZonaBuscador.class);

    private OrigenGeneral oGeneral;

    public ZonaBuscador(Frame parent, boolean modal) {
        super(parent, modal);
    }

    public ZonaBuscador(Dialog parent, boolean modal) {
        super(parent, modal);
    }

    public static ZonaBuscador mostrar(Component padre, Aplicacion app) {
        ZonaBuscador ab;
        Window mw = SwingUtilities.getWindowAncestor(padre);
        if (mw instanceof Frame) {
            ab = new ZonaBuscador((Frame) mw, true);
        } else {
            ab = new ZonaBuscador((Dialog) mw, true);
        }
        ab.initComponents();
        ab.init(app);
    ...
    private void jbBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbBuscarActionPerformed
        try {
            modelo.setList(oGeneral.listarZonas(jtfNombre.getText()));
